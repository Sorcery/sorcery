...
describe SorceryController, active_record: true, type: :controller do
  ...
  context 'with OAuth features' do
    ...
    [:github, :google, :liveid, :vk, :salesforce, :paypal, :slack, :wechat, :graph].each do |provider|
  ...
  describe 'OAuth with User Activation features' do
    before(:all) do
      ...
      sorcery_controller_property_set(:external_providers, [:facebook, :github, :google, :liveid, :vk, :salesforce, :paypal, :slack, :wechat, :graph])
      ...
      sorcery_controller_external_property_set(:graph, :key, "eYVNBjBDi33aa9GkA3w")
      sorcery_controller_external_property_set(:graph, :secret, "XpbeSdCoaKSmQGSeokz5qcUATClRW5u08QWNfv71N8")
      sorcery_controller_external_property_set(:graph, :callback_url, "http://blabla.com")
    end
    ...
    [:github, :google, :liveid, :vk, :salesforce, :paypal, :wechat, :graph].each do |provider|
  ...
  def provider_url(provider)
    {
      ...
      graph: "https://login.microsoftonline.com/common/oauth2/v2.0/authorize?client_id=#{::Sorcery::Controller::Config.graph.key}&response_type=code&redirect_uri=http%3A%2F%2Fblabla.com&response_mode=query&scope=openid%20offline_access%20https%3A%2F%2Fgraph.microsoft.com%2Fmail.read&state=12345"
    }[provider]
  end
end
